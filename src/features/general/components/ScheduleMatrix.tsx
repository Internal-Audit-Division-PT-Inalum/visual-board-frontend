import type { ScheduleZoneRow, DayStatus } from '@/types/api';
...
export function ScheduleMatrix({ data }: ScheduleMatrixProps) {
  ...
  const renderSymbol = (status?: DayStatus) => {
    switch (status) {
      case 'rencana':
        return <span className="text-slate-400 font-bold text-lg leading-none">○</span>;
      case 'ok_tanpa_5r':
        return <span className="text-emerald-500 font-extrabold text-lg leading-none">◎</span>;
      case 'ok_dengan_5r':
        return <span className="text-amber-500 font-extrabold text-lg leading-none">△</span>;
      case 'abnormal':
        return <span className="text-red-500 font-extrabold text-lg leading-none">☒</span>;
      default:
        return null;
    }
  };
  ...
  const groupedByZone = data.reduce((acc, row) => {
  ...
  return (
    <div className="flex flex-col gap-8 font-sans">
      {zoneNames.map(zoneName => {
        ...
        const groupedByItem = zoneData.reduce((acc, row) => {
        ...
        return (
          <div key={zoneName} className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200 overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-[#F1F5F9] text-slate-700 font-extrabold text-[11px] uppercase tracking-wider">
                  <tr>
                    <th className="px-4 py-3 border border-slate-300 min-w-[120px] text-center bg-[#E2E8F0]">Item 5R</th>
                    <th className="px-4 py-3 border border-slate-300 min-w-[300px] text-center bg-[#E2E8F0]">Standar</th>
                    <th colSpan={31} className="px-4 py-1 border border-slate-300 text-center bg-[#E2E8F0]">Tanggal</th>
                  </tr>
                  <tr>
                    <th className="border border-slate-300 bg-[#E2E8F0] p-0"></th>
                    <th className="border border-slate-300 bg-[#E2E8F0] p-0"></th>
                    {days.map(day => (
                      <th key={day} className="px-1 py-2 border border-slate-300 text-center w-8 min-w-[32px] bg-slate-50">
                        {day}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-slate-200">
                  {itemGroups.map(itemGroup => {
                    ...
                    return groupRows.map((row, index) => (
                      <tr key={`${row.zone_id}-${row.criteria_code}-${index}`} className="hover:bg-blue-50/30 transition-colors">
                        ...
                        <td className="px-4 py-2 border border-slate-300 text-slate-700 font-medium">
                          <span className="font-bold mr-2 text-slate-900">{row.criteria_code}</span>
                          {row.criteria_description}
                        </td>

                        
                        {days.map(day => (
                          <td key={day} className="px-1 py-1 text-center border border-slate-200">
      ...
      <div className="p-4 sm:p-5 bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 flex flex-wrap gap-6 sm:gap-8 text-xs sm:text-sm font-bold text-slate-700 justify-center">
        ...
        <div className="flex items-center gap-2">
          {renderSymbol('ok_tanpa_5r')} <span className="ml-1">= OK, Tanpa melakukan 5R</span>
        </div>
        <div className="flex items-center gap-2">
          {renderSymbol('ok_dengan_5r')} <span className="ml-1">= OK, Dengan melakukan 5R</span>
        </div>
        <div className="flex items-center gap-2">
          {renderSymbol('abnormal')} <span className="ml-1">= Ada temuan abnormality</span>
        </div>
      </div>
