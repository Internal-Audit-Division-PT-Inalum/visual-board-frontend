...
export function KaizenLeaderboard({ champions, isLoading, isError }: KaizenLeaderboardProps) {
      ...
      <div className="p-5 sm:p-6 pb-4 flex justify-between items-start shrink-0">
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[10px] sm:text-xs">Penghargaan Kinerja Terbaik</p>
          </div>
        </div>
          ...
          <div className="flex justify-center items-end h-full gap-2 sm:gap-4 max-w-md mx-auto w-full">
              ...
              {podiumOrder[0] && (
                <>
                  ...
                  <div className="text-center mb-2">
                    <div className="font-extrabold text-slate-800 text-xs sm:text-sm truncate w-24">{podiumOrder[0].name}</div>
                    <div className="text-[9px] sm:text-[10px] text-slate-400 font-medium truncate w-24">{podiumOrder[0].department}</div>
                    <div className="font-bold text-[#0A2F66] text-sm mt-1">{podiumOrder[0].score} pts</div>
                  </div>
                  <div className="w-full bg-[#F1F5F9] border-t-2 border-slate-300 rounded-t-xl h-24 sm:h-28 flex flex-col items-center justify-end pb-4 shadow-inner relative overflow-hidden group-hover:h-28 transition-all">
                     <div className="font-black text-slate-400 text-xs tracking-widest uppercase">PERAK</div>
                     <div className="text-[9px] text-slate-500 font-medium mt-1">{podiumOrder[0].kaizen_count} Kaizen</div>
                  </div>
                </>
              )}
            </div>
            ...
            <div className="flex-1 flex flex-col items-center group -mt-8 z-10">
              {podiumOrder[1] && (
                <>
                  <div className="relative mb-2">
                    <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-2xl drop-shadow-md z-20">👑</div>
                  ...
                  <div className="text-center mb-2">
                    <div className="font-black text-slate-900 text-sm sm:text-base truncate w-28">{podiumOrder[1].name}</div>
                    <div className="text-[10px] sm:text-xs text-slate-500 font-medium truncate w-28">{podiumOrder[1].department}</div>
                    <div className="font-black text-emerald-600 text-base mt-1">{podiumOrder[1].score} pts</div>
                  </div>
                  <div className="w-full bg-gradient-to-b from-[#0A2F66] to-[#041530] border-t-4 border-[#FBBF24] rounded-t-2xl h-36 sm:h-44 flex flex-col items-center justify-end pb-5 shadow-2xl relative overflow-hidden group-hover:h-44 transition-all">
                     <div className="bg-[#F59E0B] text-white px-3 py-0.5 rounded-sm text-[9px] font-black tracking-widest uppercase mb-2 shadow-sm border border-amber-300">LENCANA BINTANG</div>
                     <div className="font-black text-[#FBBF24] text-xs sm:text-sm tracking-widest uppercase text-center leading-tight">JUARA<br/>EMAS</div>
                     <div className="text-[10px] text-blue-200 font-medium mt-1.5">{podiumOrder[1].kaizen_count} Kaizen</div>
                  </div>
                </>
              )}
            </div>
            ...
            <div className="flex-1 flex flex-col items-center group">
              {podiumOrder[2] && (
                <>
                  ...
                  <div className="text-center mb-2">
                    <div className="font-extrabold text-slate-800 text-xs sm:text-sm truncate w-24">{podiumOrder[2].name}</div>
                    <div className="text-[9px] sm:text-[10px] text-slate-400 font-medium truncate w-24">{podiumOrder[2].department}</div>
                    <div className="font-bold text-[#0A2F66] text-sm mt-1">{podiumOrder[2].score} pts</div>
                  </div>
                  <div className="w-full bg-[#FFF7E6] border-t-2 border-[#D97706] rounded-t-xl h-20 sm:h-24 flex flex-col items-center justify-end pb-4 shadow-inner relative overflow-hidden group-hover:h-24 transition-all">
                     <div className="font-black text-[#D97706] text-xs tracking-widest uppercase">PERUNGGU</div>
                     <div className="text-[9px] text-orange-700/60 font-medium mt-1">{podiumOrder[2].kaizen_count} Kaizen</div>
                  </div>
                </>
              )}
            </div>
