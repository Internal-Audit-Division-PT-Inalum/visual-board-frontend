...
export function ZoneStandardImage({ zone }: ZoneStandardImageProps) {
  ...
  const imageUrl = zone.standard_image_url ? `http://localhost:8000${zone.standard_image_url}` : null;
    ...
    <div className="flex flex-col h-full bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
      ...
      {isModalOpen && imageUrl && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center p-8">
          ...
          <img src={imageUrl} alt={`Standar Zona ${zone.name}`} className="w-full h-full object-contain rounded-lg" />
        </div>
      )}
    </div>
  );
