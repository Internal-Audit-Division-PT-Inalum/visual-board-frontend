...
export default function Schedule5R() {
	...
	const { data, isLoading, refetch, isFetching } = useKioskData(
		selectedMonth,
		selectedYear,
	);

	const uniqueZoneNames = Array.from(
		new Set(data?.schedule_matrix?.map((row) => row.zone_name) || []),
	);
			...
			<div className="w-full space-y-12">
				{isLoading ? (
					...
					uniqueZoneNames.map((zoneName) => {
						const zoneData =
							data?.schedule_matrix?.filter(
								(row) => row.zone_name === zoneName,
							) || [];
						const zoneDetail = data?.zones?.find(
							(z) => z.name === zoneName,
						) || {
							id: "",
							name: zoneName,
								...
								<div className="flex flex-col sm:flex-row sm:items-center gap-4">
									...
									<div className="flex flex-col sm:flex-row gap-2 sm:gap-6 text-sm">
										<div className="flex items-center gap-2">
											...
											<span className="font-semibold text-slate-800 bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-md border border-emerald-100">
												{zoneDetail.pic_utama || "Belum Ditentukan"}
											</span>
										</div>
										<div className="flex items-center gap-2">
											<span className="font-bold text-slate-500">
											...
											<span className="font-semibold text-slate-800 bg-blue-50 text-blue-700 px-2 py-0.5 rounded-md border border-blue-100">
												{zoneDetail.pic_pengganti || "Belum Ditentukan"}
											</span>
										</div>
									</div>
