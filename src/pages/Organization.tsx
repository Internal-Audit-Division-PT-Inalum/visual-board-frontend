...
import { Loader2, AlertCircle, Building2, Users, Map } from 'lucide-react';

export default function Organization() {
  const { data, isLoading, isError } = useAttendanceSummary();
  const [activeTab, setActiveTab] = useState<'structure' | 'map_area' | 'attendance'>('structure');
    ...
    return (
      <div className="flex items-center justify-center min-h-[500px]">
        <div className="text-center p-8 bg-red-50 rounded-2xl border border-red-100 max-w-md">
          ...
          <p className="text-red-600 text-sm">Tidak dapat terhubung ke server HR. Silakan periksa koneksi atau hubungi Administrator.</p>
        </div>
      </div>
    );
  }
        ...
        {activeTab === 'attendance' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="lg:col-span-8 h-full min-h-[500px] flex flex-col gap-6">
              <DivisionEmployees employees={data.division_employees} />
            </div>
